...
main = do
  ...
  mapM_
    ...
    , "create table if not exists news (id serial primary key, date date not null, author_id integer not null references authors(id) on delete restrict, category_id integer not null references categories(id) on delete restrict, text text, main_photo varchar(999), published bool not null default false);"
    ...
    , "create table if not exists news_tags (id serial primary key, tag_id integer not null references tags(id) on delete cascade, news_id integer references news(id) on delete cascade);"
